...
import { Book, BookFormResult } from '../../interfaces/book';
...
const bookFormResult: BookFormResult = {
  id: 1,
  title: 'Clean Code',
  authorId: 1,
  isbn: '9780132520884',
...
const mockAuthorService = {
  getListOfAuthors: () => of([{ id: 1, name: 'Robert C. Martin', country: 'USA' }]),
};
...
describe('create book', () => {
  ...
  it('should initialize empty form', () => {
    ...
    expect(component.form.value['authorId']).toBe('');
    expect(component.form.value['isbn']).toBe(null);
...
describe('edit book', () => {
  ...
  it('should set the book form with dialogRef data', () => {
    ...
    expect(component.form.value['authorId']).toBe(book.authorId);
  ...
  it('on close should close the dialog and pass data', () => {
    ...
    expect(component.dialogRef.close).toHaveBeenCalledWith(bookFormResult);
  });
  ...
  it('should be invalid when author is empty', () => {
    component.form.get('authorId')?.setValue('');
    component.form.get('authorId')?.markAsTouched();
    fixture.detectChanges();
    const element = fixture.nativeElement.querySelector('mat-error');
    expect(component.form.invalid).toBe(true);
